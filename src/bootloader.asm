...
jmp short start                                                                         		 			 ; Jump to the start of the bootloader
...
MediumByte                   db                  0F0h                                              ; Medium descriptor Bytes
...
VolumeLabel                  db                  "KNIGHTOS   "                                     ; Volume Label: any 11 chars
...
start:
    ...
    mov ss, ax                                                                                     ; Setting up the Stack Segment with the value in ax
    mov sp, 4096                                                                                   ; Set the Stack Pointer to point to memory address 4096
    ...
    int 13h                                                                                        ; Interrupt call

    jc fatal_disk_error
    and cx, 3Fh                                                                                    ; Maximum sector number
    mov [SectorsPerTrack], cx                                                                      ; Sector numbers start at 1
    movzx dx, dh                                                                                   ; Maximum head number
    add dx, 13h                                                                                    ; Head numbers start at 0 - add 1 for total
    mov [Sides], dx

no_change:
    mov eax, 07C0h                                                                                 ; Needed for some older BIOSes
...
kern_filename                db                  "KERNEL  BIN"                                     ; KnightOS kernel filename
